...
- `proxyUrl` (required): Backend proxy URL. Default hosted proxy: `https://autoui-proxy.onrender.com` (or use your own proxy server URL if self-hosting)
- `sharedSecret` (optional): Shared secret for proxy authentication. Get this from the [AutoUI Service Portal](https://autoui-chi.vercel.app/) (or generate it from your own proxy server if self-hosting)
