...
<Mermaid
  chart={`sequenceDiagram
    ...
    ModalChat->>Proxy: Send message + appId + schema metadata
    Proxy->>OpenRouter: Forward request (server-side OpenRouter key)
    OpenRouter-->>Proxy: Return plan JSON
    Proxy-->>Runtime: Return plan JSON
    ...
    Runtime-->>ModalChat: Render assistant response + UI actions
  `} 
/>
