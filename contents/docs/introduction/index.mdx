...
<StepItem title="2. Request + Config Transmission">
ModalChat sends a request to the proxy server that typically includes:
- User message
- `appId` + `sharedSecret` (client credential)
- A properly formatted prompt built from your `AutoUIConfig` (metadata, functions, components, runtime settings)
- Optional schema metadata (so the backend/LLM understands available types)
</StepItem>

<StepItem title="3. Proxy Auth + LLM Processing">
The proxy server:
- Verifies that the `appId` exists and that the `sharedSecret` is valid
- Decrypts the stored OpenRouter API key (stored encrypted in the database, so your key is not exposed to the frontend)
- Forwards the request to OpenRouter/LLM

The LLM returns a structured **instruction plan** (JSON) describing what actions to take.
</StepItem>
...
Think of the runtime as a small **engine**: it runs the instruction plan and maintains a **context store** (key → value pairs) that can be used across steps.
</StepItem>

<StepItem title="5. extraAnalysis (selective data sharing)">
Some function results may be safe to send back to the LLM. If a function is marked with `canShareDataWithLLM`, the runtime can build an `extraAnalysis` payload from those results (plus relevant context) and send it back through the proxy to improve the next LLM turn.
</StepItem>
</Step>
...
- Maintains an internal **context store** (key → value pairs) used during plan execution and follow-up turns
...
1. **Use AutoUI's hosted portal + proxy** (Recommended for quick start): Visit the **[AutoUI Service Portal](https://autoui-chi.vercel.app/)** to register an app (you’ll paste your OpenRouter API key there). You’ll receive an `appId` and `sharedSecret`. By default, your frontend should use `https://autoui-proxy.onrender.com` as `llm.proxyUrl`.
...
<Mermaid
  chart={`sequenceDiagram
    ...
    participant ModalChat as ModalChat_Frontend
    participant Proxy as Proxy_Backend
    participant OpenRouter as OpenRouter_LLM
    participant Runtime as AutoUI_Runtime
    participant App as YourApp_Functions_UI

    User->>ModalChat: Send message
    ModalChat->>Proxy: Send message + appId + sharedSecret + prompt/config + schema metadata
    Proxy->>Proxy: Verify appId + sharedSecret (DB)
    Proxy->>Proxy: Decrypt OpenRouter API key (stored encrypted)
    Proxy->>OpenRouter: Forward request (server-side OpenRouter key)
    OpenRouter-->>Proxy: Return instructionPlan JSON
    Proxy-->>Runtime: Return instructionPlan JSON
    Runtime->>Runtime: Validate plan (optional, schema-based)
    Runtime->>Runtime: Execute plan engine + update context (key->value)
    Runtime->>App: Call functions / render components
    App-->>Runtime: Return results (data + UI state)
    Runtime->>Runtime: Build extraAnalysis (only canShareDataWithLLM results)
    Runtime-->>Proxy: Send extraAnalysis (optional)
    Runtime-->>ModalChat: Render UI updates + assistant response
  `} 
/>
...
└──────┬──────────┘
       │
       │ Verifies appId/sharedSecret, decrypts OpenRouter key, forwards to LLM
       ▼
┌─────────────────┐
│  OpenRouter     │
│  / LLM Provider │
...
Ready to get started? Head to the [Quick Start](/docs/quickstart) for the fastest setup, or use the [Complete Developer Guide](/docs/developer-guide) for deeper explanations.
